    ...
    if(empty($required)){
        ...
        if($attender->signup()){
            ...
            if(!empty($user)){
                ...
                if(isset($post['shift_id']) && !empty($post['shift_id'])){
                    $shifts=explode(',', $post['shift_id']);
                    $addupdateAttender=DrsPanel::addUpdateAttenderToShifts($shifts,$user->id);
                }
    ...
    if(empty($required)){
        if(isset($post['id']) && !empty($post['id'])){
            ...
            if($attender->save() && $userProfile->save()) {
                ...
                if(isset($post['shift_id']) && !empty($post['shift_id']) ){
                    $shifts=explode(',',$post['shift_id']);
                    $addupdateAttender=DrsPanel::addUpdateAttenderToShifts($shifts,$attender->id);
                }else{
...
public function actionDoctorShiftList(){
    ...
    Yii::info($post, __METHOD__);
    if(empty($required)) {
        $search['user_id']=$post['doctor_id'];
        if(isset($post['address_id']) && !empty($post['address_id']))
        {
            $search['address_id']=$post['address_id'];
        }
        $shifts=Drspanel::shiftList($search,'apilist');
        if($shifts){
            $response["status"] = 1;
            $response["error"] = false;
            $response['message']= 'Success';
